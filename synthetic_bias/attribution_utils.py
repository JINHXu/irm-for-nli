...
def construct_input_ref_pair(tokenizer, text, device='cpu'):
    ...
    position_ids = torch.tensor([[i for i in range(seq_len)]
    ...
    ref_input_ids[torch.bitwise_and(
        ref_input_ids != sep_token_id, ref_input_ids != cls_token_id)] = ref_token_id
...
def get_bias_indices(input_ids, tokenizer):
    ...
    tokens_len = [len(tokenizer(x)['input_ids']) -
                  2 for x in settings.VOCAB_BIAS[0]]
    assert len(set(
        tokens_len)) == 1, 'Different bias tokens are tokenizer to different number of subtokens'
    bias_ind = []
...
def summarize_attributions(attributions):
    ...
    attributions = attributions / \
        torch.norm(attributions, p=2, dim=1).unsqueeze(1)
    return attributions
...
def attributions_cm(attr, y_gt, y, bias_indices, num_labels):
    ...
    # B x 3 ( bias token is tokenized to 3 : [<, char, >])
    bias_attr = attr[torch.arange(B).unsqueeze(-1), bias_indices]
    bias_attr = torch.sum(bias_attr, dim=-1)  # B
    with wrap_confusion_matrix(num_labels, y_gt, y, bias_attr) as padded_inputs:
        cm = confusion_matrix(
            y_true=padded_inputs[0], y_pred=padded_inputs[1], sample_weight=padded_inputs[2])
    return cm
...
def confusion_matrix_from_embedding_attribution(model, tokenizer, dl, device='cpu', internal_bs=None):
    ...
    interpretable_word_embedding = configure_interpretable_embedding_layer(
    ...
    attribution_cm_dict, cm = {
        'gold label': None, 'predicted label': None}, None
    ...
    for batch in dl:
        ...
        inputs, ref_inputs, attention_mask = construct_input_ref_pair(
            tokenizer, batch[0], batch[1], device)
        ...
        word_embeddings = interpretable_word_embedding.indices_to_embeddings(
            inputs[0])
        ref_word_embeddings = interpretable_word_embedding.indices_to_embeddings(
            ref_inputs[0])
        # ref_word_embeddings = torch.zeros_like(word_embeddings).to(device=device)
        tokentype_embeddings = interpretable_tokentype_embedding.indices_to_embeddings(
            inputs[1])
        position_embeddings = interpretable_position_embedding.indices_to_embeddings(
            inputs[2])

        # attributions per label
        with torch.no_grad():
            logits = predict(word_embeddings, tokentype_embeddings,
        ...
        for k, targets in zip(['predicted label', 'gold label'], [preds, batch[-1]]):
            ...
            expanded_attention_mask = attention_mask.to(
            ...
            if attribution_cm_dict[k] is None:
                attribution_cm_dict[k] = attributions_cm(
                    attributions, batch[-1], preds, bias_ind, num_labels)
            else:
                attribution_cm_dict[k] += attributions_cm(
                    attributions, batch[-1], preds, bias_ind, num_labels)
            bias_attr = attributions[torch.arange(
                len(bias_ind)).unsqueeze(-1), bias_ind]
    ...
    for k in ['predicted label', 'gold label']:
        attribution_cm_dict[k] = np.around(
            attribution_cm_dict[k] / (cm + np.ones_like(cm) * 1e-5), 2)

    remove_interpretable_embedding_layer(model, interpretable_word_embedding)
    remove_interpretable_embedding_layer(
        model, interpretable_tokentype_embedding)
    remove_interpretable_embedding_layer(
        model, interpretable_position_embedding)
...
def heat_map_from_embedding_attribution1(model, tokenizer, samp, device='cpu'):
    ...
    for lbl, label in enumerate(labels):
        label_attributions = ig.attribute(inputs=(input_ids, tokentype_ids, position_ids),
                                          baselines=(
                                              ref_input_ids, ref_tokentype_ids, ref_position_ids),
                                          additional_forward_args=(
                                              attention_mask,),
                                          target=torch.ones_like(y, device=device) * lbl, n_steps=30).detach().to('cpu')
        # tensor: 1 x S x h
        label_attributions = summarize_attributions(
            label_attributions)  # 1 x S x h => 1 x S
        attr_dict[label] = label_attributions

    fig = attribution_to_heatmap_figure(
        attr_dict, tokenizer.convert_ids_to_tokens(inputs[0].squeeze()))
...
def heat_map_from_embedding_attribution2(model, tokenizer, samp, device='cpu'):
    ...
    # p, h, y = samp  # p, h : strings. y: tensor, shape []
    # labels = ['contradiction', 'entailment', 'neutral']
    t, y = samp  # p, h : strings. y: tensor, shape []
    labels = ['hate', 'neutral']
    labels.sort()
...
def heat_map_from_sub_embedding_attribution(model, tokenizer, samp, device='cpu'):
    ...
    for lbl, label in enumerate(labels):
        label_attributions = ig.attribute(inputs=input_embeddings, baselines=ref_input_embeddings,
                                          additional_forward_args=(
                                              inputs[1], inputs[2], attention_mask),
